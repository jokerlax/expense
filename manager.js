...
function navigateToTab(tabName) {
  ...
  const sections = ["team-dashboard", "approvals", "approval-history", "my-dashboard", "add-expense", "expense-reports", "report-download", "my-expenses", "settlements"];
  sections.forEach(s => {
...
// ==============================================================================
// 12. REPORT & ATTACHMENT DOWNLOAD DESK CONTROLLERS (MANAGER PORTAL)
// ==============================================================================
...
function initReportDownloadPage() {
  ...
  function loadAndRenderData() {
    ...
    downloadExpensesList = allExp.filter(e => e.status !== "DRAFT" && e.status !== "CANCELLED");
    ...
    populateDownloadEmployeeFilter();
  ...
  if (typeof fetch === "function") {
    fetch("/api/data/expenses")
      ...
      .catch(err => console.warn("[Manager Report Download] live expenses sync notice:", err));
  }
}

function populateDownloadEmployeeFilter() {
  const empSelect = document.getElementById("dl-exp-filter-employee");
  if (!empSelect) return;
  ...
  let html = `<option value="ALL">All Employees (${empMap.size})</option>`;
  html += `<option value="MY_CLAIMS">My Personal Claims</option>`;
  html += `<option value="TEAM_CLAIMS">Team Claims Only</option>`;

  empMap.forEach((name, id) => {
    const isMe = currentUser && (currentUser.employeeId === id || currentUser.name === name);
    html += `<option value="${id}">${name} ${isMe ? '(Me)' : ''}</option>`;
  });
...
function populateDownloadCategoryFilter() {
  ...
  const categories = expenseDb.getExpenseCategories ? expenseDb.getExpenseCategories() : (expenseDb.getTable("categories") || []);
...
function renderMonthWiseDownload() {
  ...
  downloadExpensesList.forEach(e => {
    ...
    if (!monthMap[ym]) {
      monthMap[ym] = {
        ...
        expenses: [],
        totalAmount: 0,
        currency: e.currency || getUserCurrency(),
        receiptCount: 0,
        statusCounts: {}
      };
    }
  ...
  sortedMonthKeys.forEach(ym => {
    ...
    const statusBadges = Object.keys(mData.statusCounts).map(st => {
      ...
      return `<span class="status-badge ${cls}" style="font-size: 10px; margin-right: 4px; padding: 2px 6px;">${count} ${(typeof formatStatusLabel === 'function' ? formatStatusLabel(st) : st.replace(/_/g, ' '))}</span>`;
    }).join("");
...
function filterExpenseDownload() {
  ...
  const emp = document.getElementById("dl-exp-filter-employee")?.value || "ALL";
  ...
  filteredDownloadExpenses = downloadExpensesList.filter(e => {
    if (emp === "MY_CLAIMS" && e.employeeId !== currentUser.employeeId) return false;
    if (emp === "TEAM_CLAIMS" && e.employeeId === currentUser.employeeId) return false;
    if (emp !== "ALL" && emp !== "MY_CLAIMS" && emp !== "TEAM_CLAIMS" && e.employeeId !== emp) return false;
    ...
    if (status !== "ALL") {
      ...
      else if (status !== "PENDING" && st !== status) return false;
    }
...
function resetExpenseDownloadFilters() {
  ...
  if (document.getElementById("dl-exp-filter-employee")) document.getElementById("dl-exp-filter-employee").value = "ALL";
  ...
  filteredDownloadExpenses.forEach(exp => {
    ...
    html += `
      <tr>
        ...
        <td>
          <div style="font-weight: 600; font-size: 12px; color: var(--text-main);">${exp.employeeName || exp.employeeId || 'Unknown'}</div>
          <div style="font-size: 10.5px; color: var(--text-muted);">${exp.employeeId || ''}</div>
        </td>
        <td>
        ...
        <td>
          <div style="font-size: 12px; max-width: 220px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${exp.description || ''}">
        ...
        <td>
          <span class="status-badge ${statusCls}">${(typeof formatStatusLabel === 'function' ? formatStatusLabel(st) : st.replace(/_/g, ' '))}</span>
        </td>
        <td style="text-align: center;">
          ${receiptSrc 
...
function updateExpenseBatchBar() {
  ...
  if (count > 0) {
    ...
    const curr = selectedExps[0]?.currency || getUserCurrency();
...
function updateDownloadKPIs() {
  ...
  const mainCurr = exps[0]?.currency || getUserCurrency();
...
function buildExpensesExcelWorkbook(expenses, title = "Expense Report") {
  ...
  const rows = expenses.map(e => ({
    ...
    "Status": typeof formatStatusLabel === 'function' ? formatStatusLabel(e.status || "UNREPORTED") : (e.status || "UNREPORTED").replace(/_/g, ' '),
  ...
  if (options.includeReceipts) {
    ...
    if (receiptExpenses.length > 0) {
      for (let i = 0; i < receiptExpenses.length; i++) {
        ...
        const empPart = exp.employeeName ? `_${exp.employeeName.replace(/\s+/g, "_")}` : "";
        const baseName = `Receipt_${exp.id}${empPart}_${(exp.category || "Expense").replace(/\s+/g, "_")}`;
...
function previewReceiptModal(exp) {
  ...
  if (info) info.innerText = `${exp.employeeName ? exp.employeeName + ' | ' : ''}${exp.category} | ${formatAmount(exp.amount, exp.currency)} | ${formatDate(exp.date)}`;
  if (dlBtn) {
    dlBtn.href = src;
...
async function buildExpensesPDFDoc(expenses, options = {}) {
  ...
  doc.setFontSize(8.5);
  doc.setFont("helvetica", "normal");
  doc.text(`Generated: ${today} | Manager: ${currentUser?.name || 'Manager'} (${currentUser?.employeeId || '-'})`, 14, 17.5);
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  const metaText = `Scope: ${options.scopeLabel || 'Manager Expense Download'}   |   Department: ${currentUser?.department || 'Management'}   |   Currency: ${userCurrency}   |   Total Value: ${formatAmount(totalAmount, userCurrency)}   |   Claims: ${expenses.length}`;
  doc.text(metaText, 18, 33.5);

  // Table Data
  const headers = ["Date", "Expense ID", "Employee", "Category", "Description", "Project", "Payment", "Status", "Amount", "Receipt"];
  const rows = expenses.map(e => [
    e.date || "",
    e.id || "",
  ...
  if (options.includeReceiptPages !== false) {
    ...
    for (let i = 0; i < receiptExpenses.length; i++) {
      ...
      doc.text(`Payment: ${exp.paymentMethod || 'Corporate'}   |   Employee: ${exp.employeeName || '-'} (${exp.employeeId || '-'})`, 18, 42.5);
